...
def evol_loop(arg1: str) -> None:
    ...
    try:
        ...
        # Población de un solo individuo para prueba fija.
        fixed_pid = np.array(
            [
                30.25611318, 15.57888127, 0.59747982,
                230.98884181, 16.94922700, 0.45489331,
                145.29594935, 19.64660100, 0.63531273,
                16.41774398, 13.49640039, 0.84791067,
                34.49991100, 10.77804730, 0.40561126,
            ],
            dtype=np.float64,
        )

        # gen_population crea la estructura con las columnas
        # correspondientes a ganancias, OF y SCV.
        X = de.gen_population()
        ...
        with output_path.open(
            ...
            while rclpy.ok():
                ...
                de.node.get_logger().info(
                    f'OF recalculado: {result[de.m]}'
                )

                de.node.get_logger().info(
                    f'SCV recalculado: {result[de.m + 1]}'
                )
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        raise SystemExit(
            'Uso: test_fixed_pid.py <identificador>'
        )
